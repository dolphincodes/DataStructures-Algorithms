...
// returning all possible choices to make target sum by using suffix of array
// [curIndex, ...]
vector<choice> getAllChoices(int curIndex, int target) {
  ...
  if (target == 0)
    return {{}}; // one choice, and you chose nothing
  ...
  vector<choice> ans =
      getAllChoices(curIndex + 1, target); // curNumber is not used at all

  vector<choice> other =
      getAllChoices(curIndex, target - curNumber); // using it once
  for (choice c : other) {
